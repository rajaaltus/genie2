    ...
    <v-card class="mx-auto mb-1" outlined>
      <v-list-item three-line>
        <v-list-item-content>
          ...
          <p class="body-2">{{info.faculty_name}}, {{info.designation}} participated in {{info.forum}} programme titled "{{info.program_name}}" from {{ $moment(info.from_date).format("DD MMM YYYY") }} to {{ $moment(info.to_date).format("DD MMM YYYY") }} at {{info.place}}.</p>
  ...
  methods: {
    ...
    handleApprove() {
      ...
      this.$store.dispatch("participation/updateParticipation", payload).then(resp => {
        ...
        this.$store.dispatch("participation/setParticipationsData", { qs: queryString });
        Swal.fire({
          title: "Success",
    ...
    handleDelete() {
      ...
      Swal.fire({
        ...
        if (!result.dismiss) {
          this.$store
            .dispatch("participation/deleteParticipation", { id: id })
            .then(response => {
              Swal.fire({
                type: "success",
