    ...
    <v-card class="mx-auto mb-1" outlined>
      <v-list-item three-line>
        <v-list-item-content>
          ...
          <p class="body-2 font-weight-medium">{{info.status.toUpperCase()}}: {{info.student_name}}: {{info.thesis_title}}</p>
          <p class="body-2"><b>Guide: </b>{{info.guide}}, <b>Co-Guides: </b>{{info.coguides}}</p>
  ...
  methods: {
        ...
        if (!result.dismiss) {
          this.$store
            ...
            .then(response => {
              ...
              let queryString = "";
              queryString = `department.id=${this.$store.state.auth.user.department}&approval_status=Pending&deleted_ne=true`;
              this.$store.dispatch("theses/setThesesData", {
                qs: queryString
              });
            });
        }
    ...
    handleDelete() {
      ...
      Swal.fire({
        ...
        if (!result.dismiss) {
          this.$store
            .dispatch("theses/deleteTheses", { id: id })
            .then(response => {
              Swal.fire({
                type: "success",
