    ...
    <v-card class="mx-auto mb-1" outlined>
      <v-list-item three-line>
        <v-list-item-content>
          ...
          <p class="body-2">{{ info.name }}, {{ info.designation }} from {{ info.institutional_affiliation }} visited to our department during from {{ $moment(info.from_date).format("DD MMM YYYY") }} to {{ $moment(info.to_date).format("DD MMM YYYY") }} and has given lecture on "{{ info.title }}".</p>
  ...
  methods: {
        ...
        if (!result.dismiss) {
          this.$store
            ...
            .then(response => {
              ...
              let queryString = "";
              queryString = `department.id=${this.$store.state.auth.user.department}&approval_status=Pending&deleted_ne=true`;
              this.$store.dispatch("visitor/setVisitorsData", {
                qs: queryString
              });
            });
        }
    ...
    handleDelete() {
      ...
      Swal.fire({
        ...
        if (!result.dismiss) {
          this.$store
            .dispatch("visitor/deleteVisitor", { id: id })
            .then(response => {
              Swal.fire({
                type: "success",
