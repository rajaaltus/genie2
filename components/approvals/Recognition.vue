    ...
    <v-card class="mx-auto mb-1" outlined>
      <v-list-item three-line>
        <v-list-item-content>
          ...
          <p class="font-weight-bold">{{info.award_title}}</p>
          <p class="body-2 font-weight-medium">{{info.faculty_name}} has been awarded as "{{info.award_title}}" by {{info.organization}}, {{info.place}} on {{ $moment(info.date).format("DD MMM YYYY") }}.</p>
  ...
  methods: {
    ...
    handleApprove() {
      ...
      this.$store.dispatch("recognition/updateRecognition", payload).then(resp => {
        ...
        this.$store.dispatch("recognition/setRecognitionsData", { qs: queryString });
        Swal.fire({
          title: "Success",
    ...
    handleDelete() {
      ...
      Swal.fire({
        ...
        if (!result.dismiss) {
          this.$store
            .dispatch("recognition/deleteRecognition", { id: id })
            .then(response => {
              Swal.fire({
                type: "success",
