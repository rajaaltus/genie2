      ...
      <v-list-item three-line>
        <v-list-item-content>
          ...
          <!-- Main Content -->
					...
					<p class="body-1 font-weight-normal">{{ info.reference }}</p>
...
export default {
  ...
  methods: {
    ...
    handleApprove() {
      ...
      this.$store.dispatch("publication/updatePublication", payload).then(resp => {
        let queryString = "";
        queryString = `department.id=${this.$store.state.auth.user.department}&approval_status=Pending&deleted_ne=true`;
        this.$store.dispatch("publication/setPublicationsData", { qs: queryString });
        Swal.fire({
          title: "Success",
    ...
    handleDelete() {
      ...
      Swal.fire({
        ...
        if (!result.dismiss) {
          this.$store
            .dispatch("publication/deletePublication", { id: id })
            .then(response => {
              Swal.fire({
                type: "success",
