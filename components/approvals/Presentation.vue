    ...
    <v-card class="mx-auto mb-1" outlined>
      <v-list-item three-line>
        <v-list-item-content>
          ...
          <p class="body-2">{{info.forum}} {{info.type}} titled "{{info.title}}"" by {{info.faculty_name}}.</p>
          <p class="body-2"><b><u>Co-author(s):</u></b> {{info.coauthors}}</p>
          <p class="body-2 font-weight-medium"><b><u>Reference</u></b></p>
					<p class="body-2 font-weight-normal">{{ info.reference }}</p>
  ...
  methods: {
    ...
    handleApprove() {
      ...
      this.$store.dispatch("presentation/updatePresentation", payload).then(resp => {
        ...
        this.$store.dispatch("presentation/setPresentationsData", { qs: queryString });
        Swal.fire({
          title: "Success",
    ...
    handleDelete() {
      ...
      Swal.fire({
        ...
        if (!result.dismiss) {
          this.$store
            .dispatch("presentation/deletePresentation", { id: id })
            .then(response => {
              Swal.fire({
                type: "success",
