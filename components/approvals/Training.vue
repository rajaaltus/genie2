    ...
    <v-card class="mx-auto mb-1" outlined>
      <v-list-item three-line>
        <v-list-item-content>
          ...
          <p class="body-2">{{ info.faculty_name}} has attended a training programme on {{info.program_name}} at {{info.institutional_affiliation}} from {{ $moment(info.from_date).format("DD MMM YYYY") }} to {{ $moment(info.to_date).format("DD MMM YYYY") }},funded by {{info.funded_by}}.</p>
  ...
  methods: {
        ...
        if (!result.dismiss) {
          this.$store
            ...
            .then(response => {
              ...
              let queryString = "";
              queryString = `department.id=${this.$store.state.auth.user.department}&approval_status=Pending&deleted_ne=true`;
              this.$store.dispatch("training/setTrainingsData", {
                qs: queryString
              });
            });
        }
    ...
    handleDelete() {
      ...
      Swal.fire({
        ...
        if (!result.dismiss) {
          this.$store
            .dispatch("training/deleteTraining", { id: id })
            .then(response => {
              Swal.fire({
                type: "success",
