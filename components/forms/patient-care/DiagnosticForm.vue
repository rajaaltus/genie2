  ...
  <div>
    <v-row no-gutters>
      <v-col cols="12" sm="6" md="8">
        <h3><span class="frm-title">Diagnostic Services</span></h3>
      </v-col>
    </v-row>
    <v-form ref="form" v-model="valid" lazy-validation @submit.prevent>
      ...
      <v-row no-gutters>
        ...
        <v-btn medium color="#57a727" dark @click="addDiagnostic">
          Submit
        </v-btn>
      </v-row>
    </v-form>
    ...
    async addTest () {
		...
		async addDiagnostic () {
			if (this.$refs.form.validate()) {
			 ...
			 await this.$store.dispatch('diagnostic/addDiagnostic', payload)
					.then(resp => {
						Swal.fire({
							title: 'Success',
